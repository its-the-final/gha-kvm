...
RUN ( echo "deb [signed-by=/etc/apt/keyrings/nodesource.gpg] https://deb.nodesource.com/node_$NODE_MAJOR.x nodistro main" |  tee /etc/apt/sources.list.d/nodesource.list ) && \
    ...
    ( echo 'set man-db/auto-update false' | debconf-communicate ) || true && \
...
#1.5G w/o qemu
#RUN apt-get update && (apt-get install -y --no-install-recommends  man-db dropbear openssl iproute2 netcat-openbsd cloud-image-utils  wget curl sudo  curl qemu-utils docker-compose-plugin socat docker.io docker-compose python3-pip nodejs python3-venv python3-full  cython3  jq bash build-essential autoconf automake  git  unzip golang curl  libblas3 libffi-dev zlib1g-dev libssl-dev nbdkit-plugin-dev make  libfuse3-dev nbdkit libssl-dev nbd-client git 2>&1|grep -v "Get:") && (apt-get clean all  ||true) && which jq && which git && which curl  
#2G w/qemu
RUN apt-get update && ( apt-get install -y --no-install-recommends  man-db dropbear openssl iproute2 netcat-openbsd cloud-image-utils  wget curl sudo  curl qemu-utils docker-compose-plugin socat docker.io docker-compose python3-pip nodejs python3-venv python3-full  cython3  jq bash build-essential autoconf automake  git  unzip golang curl  libblas3 libffi-dev zlib1g-dev libssl-dev nbdkit-plugin-dev make  libfuse3-dev nbdkit libssl-dev nbd-client git qemu-system-x86  i3-wm qemu-system-gui x11-utils ffmpeg ovmf ffmpeg qemu-system-modules-opengl gdb 2>&1|grep -v "Get:") && (apt-get clean all  ||true) && which jq && which git && (             echo "cache_will_fail_if_not" > /var/cache/apt/archives/nonexisitent.deb || true ) && which curl   
